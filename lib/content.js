...
// writing: { prompt, imageUrl? }
//   imageUrl (on a reading section or on writing) shows an image above the
//   text — e.g. a Writing Task 1 diagram/chart, or a figure referenced by a
//   reading passage. Same URL rules as audioUrl: local /images/... paths
//   are most reliable; Google Drive share links are auto-converted.
//
...
export function normalizeContent(raw) {
  ...
  const writingImageUrl = hasWriting && typeof raw.writing.imageUrl === "string" && raw.writing.imageUrl.trim()
    ? toDriveDirectUrl(raw.writing.imageUrl)
    : "";
  return {
    ...
    writing: hasWriting ? { prompt: raw.writing.prompt, imageUrl: writingImageUrl } : null,
  };
}
...
export function withIds(content) {
  return {
    ...
    writing: content.writing ? { prompt: content.writing.prompt, imageUrl: content.writing.imageUrl || "" } : null,
  };
}
