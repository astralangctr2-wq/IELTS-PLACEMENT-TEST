...
export async function getDefaultContentBank() {
  await ensureSchema();
  await seedIfEmpty();
  const { rows } = await sql`SELECT id, name, content, category FROM content_banks WHERE is_default = TRUE LIMIT 1`;
  if (rows[0]) return rows[0];
  const { rows: any } = await sql`SELECT id, name, content, category FROM content_banks ORDER BY created_at ASC LIMIT 1`;
  return any[0] || null;
}
