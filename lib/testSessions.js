...
export async function createSession({ name, skills, timeLimits, listeningPlays }) {
  await ensureSchema();
  ...
  await sql`
    INSERT INTO test_sessions (id, name, skills, time_limits, active)
    VALUES (
...
export async function listSessions() {
  await ensureSchema();
  const { rows } = await sql`SELECT id, name, skills, time_limits, active, created_at FROM test_sessions ORDER BY created_at DESC`;
  return rows;
}

export async function getSession(id) {
  await ensureSchema();
  const { rows } = await sql`SELECT id, name, skills, time_limits, active FROM test_sessions WHERE id = ${id} LIMIT 1`;
  return rows[0] || null;
}
...
// the test runner expects: { name, skills: [...], timeLimits: {...}, listeningPlays }
